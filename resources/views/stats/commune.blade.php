...
@section('title', 'Statistiques par Commune')

@section('breadcrumb')
    <span class="text-gray-400">Statistiques</span>
    <i class="fas fa-chevron-right text-xs text-gray-400"></i>
    <span class="text-gray-900 font-semibold">Communes</span>
@endsection
...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Statistiques par Commune</h1>
        ...
        <div class="flex flex-col sm:flex-row gap-2 sm:items-center">
            ...
            <form method="GET" action="{{ route('stats.commune') }}" class="inline-flex items-center gap-2">
            ...
            <form method="GET" action="{{ route('stats.commune') }}" class="flex items-center gap-2">
                ...
                <select name="commune_id"
                    ...
                    @foreach(($communes ?? []) as $d)
                        <option value="{{ $d->id }}"
                            {{ (int)($communeId ?? request('commune_id')) === (int)$d->id ? 'selected' : '' }}>
                            {{ $d->nom }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('export.commune.csv', ['commune_id' => ($communeId ?? request('commune_id'))]) }}"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-white rounded-xl shadow-sm p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Arrondissements (Participation)</h3>
            <div style="height: 320px;">
                <canvas id="arrondissementsChart"></canvas>
            </div>
        </div>

        <div class="bg-white rounded-xl shadow-sm p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Couverture par Arrondissement</h3>
            <div style="height: 320px;">
                <canvas id="couvertureArrondissementsChart"></canvas>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-900">Arrondissements de la Commune</h3>
                <a href="{{ route('export.commune.arrondissements.csv', ['commune_id' => ($communeId ?? request('commune_id'))]) }}"
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200" id="arrondissementsTable">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach(($stats['par_arrondissement'] ?? []) as $a)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="font-medium text-gray-900">{{ $a['nom'] ?? '-' }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ number_format($a['nombre_pv_valides'] ?? 0) }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <span class="text-blue-600 font-semibold">{{ number_format($a['inscrits_cena'] ?? 0) }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ number_format($a['inscrits_comptabilises'] ?? 0) }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 py-1 text-xs font-semibold rounded-full
                                {{ ($a['couverture_saisie'] ?? 0) >= 80 ? 'bg-benin-green-100 text-benin-green-800' :
                                   (($a['couverture_saisie'] ?? 0) >= 50 ? 'bg-benin-yellow-100 text-benin-yellow-800' : 'bg-benin-red-100 text-benin-red-800') }}">
                                {{ number_format($a['couverture_saisie'] ?? 0, 1) }}%
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ number_format($a['nombre_votants'] ?? 0) }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-1 bg-gray-200 rounded-full h-2 mr-2" style="width: 110px;">
                                    <div class="bg-benin-green-500 h-2 rounded-full"
                                         style="width: {{ min((float)($a['taux_participation_global'] ?? 0), 100) }}%"></div>
                                </div>
                                <span class="text-sm font-semibold text-gray-900">{{ number_format($a['taux_participation_global'] ?? 0, 2) }}%</span>
                            </div>
                        </td>
                    </tr>
                    @endforeach
                    @if(empty($stats['par_arrondissement']))
...
// Graphique Arrondissements (Participation)
const arrLabels = [
@foreach(($stats['par_arrondissement'] ?? []) as $x)
    @if(isset($x['nom'])) '{{ $x['nom'] }}', @endif
@endforeach
];

const arrParticipation = [
@foreach(($stats['par_arrondissement'] ?? []) as $x)
    {{ (float)($x['taux_participation_global'] ?? 0) }},
@endforeach
];

const ctxArr = document.getElementById('arrondissementsChart');
if (ctxArr && typeof Chart !== 'undefined') {
    new Chart(ctxArr.getContext('2d'), {
        type: 'bar',
        data: {
            labels: arrLabels,
            datasets: [{
                label: 'Participation (%)',
                data: arrParticipation,
...
// Graphique Couverture Arrondissements
const arrCouverture = [
@foreach(($stats['par_arrondissement'] ?? []) as $x)
...
const ctxCov = document.getElementById('couvertureArrondissementsChart');
